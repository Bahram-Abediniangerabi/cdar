BinomialTree_RO_MC_Figures2 <-
function(S, I, Time, r, sigma, dt, MC_loops)
{
  options(warn=-1)
  ###BinomialTree_RO_MC_Figures(S=10, I=100, Time=10, r=.01, sigma=0.6, dt=1, MC_loops = 1000)
  # Parameters:
  n = (Time / dt)+1
  u  = exp(sigma*sqrt(dt))
  d  = 1 / u
  ...
  for(i in 0:n+1){
    for(j in 0:n+1){
      if(i>j){Tree_min[i,j]=0
      }else{
        Tree_min[i,j]=Tree_min[i,j]
      }
    }
  }
  colnames(Tree_min) <- paste(0:n, sep = "")
  rownames(Tree_min) <- paste( 0:n, sep = "")
  Tree_min = round(Tree_min, digits = 2)
  #########################
  #DecisionTree Matrix
