BinomialTree_MC_RO_Fig <-
function(S, I, Time, r, sigma, dt, MC_loops)
{
  #BinomialTree_MC_RO_Fig(S=10, I=100, Time=10, r=.01, sigma=0.6, dt=1, MC_loops = 1000)
  options(warn=-1)
  # Parameters:
  n = (Time / dt)+1
  u  = exp(sigma*sqrt(dt))
  ...
  Tree_=matrix(0, nrow = n+1, ncol = n+1)
  for(i in n:1){
    for(j in n:1){
      Tree[i,j] = I-((p*Cashflow[i,j+1])+((1-p)*Cashflow[i+1,j+1]))/(1+r)
    }}
  col_Expected_Value = colMeans(Cashflow,dims = 1)
  least_value=vector(length = (length(col_Expected_Value)-2))
  for(i in 1:(n-1)){
    least_value[i]=I-(col_Expected_Value[i+1]/(1+r))}
